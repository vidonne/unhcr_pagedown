...
```{r single-line, echo=FALSE, message=FALSE, fig.cap="Plot caption example", fig.align='center'}
ggplot(datasets::iris, aes(x = Petal.Length, y = Petal.Width)) +
  ...
  unhcrthemes::theme_unhcr(base_size = 14, plot_margin = ggplot2::margin(15, 15, 15, 15))
```
...
The size of the plots can be defined directly in the code chunk using the `out.width` parameter. We recommend that you use relative measure like `out.width="75%"`. You can also set the alignment of the figure in the code chunk parameter using the `fig.align` tag, where you set it to 'left', 'center' or 'right'.
